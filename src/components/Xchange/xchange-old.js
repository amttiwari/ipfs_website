...
export default class Xchange extends Component {
    ...
    graphDataClick() {
        setTimeout(() => {
            ...
            axios.post(`/backend/api/users/graph_data`, {
    ...
    onChange(e) {
        ...
        if (e.target.name === 'token_value' || e.target.name === 'clerk_usd_price' || e.target.name === 'rcr_usd_price') {
            this.defaultAddress();
            e.target.value = (e.target.value === '') ? 0 : e.target.value;
            if (this.state.clerk_Rcr === 'RCR') {
                this.state.eth_value = parseFloat(e.target.value) * parseFloat(this.state.exchangeList?.rcr_eth_value)
            } else {
                this.state.eth_value = parseFloat(e.target.value) * parseFloat(this.state.exchangeList?.clerk_eth_value)
            }
        ...
        if (e.target.name === 'eth_value') {
            ...
            e.target.value = (e.target.value === '') ? 0 : e.target.value;
            if (this.state.clerk_Rcr === 'RCR') {
                this.state.token_value = Math.floor(parseFloat(e.target.value) / parseFloat(this.state.exchangeList?.rcr_eth_value))
            } else {
                this.state.token_value = Math.floor(parseFloat(e.target.value) / parseFloat(this.state.exchangeList?.clerk_eth_value))
            }
    ...
    async submitFormBuyOrder(e) {
        ...
        if (this.state.clerk_Rcr === 'RCR') {
            var price = this.state.rcr_usd_price;
        } else {
            var price = this.state.clerk_usd_price;
        }
        ...
        setTimeout(() => {
            ...
            if (parseFloat(this.state.eth_value) > parseFloat(this.state.ETHBalance1)) {
                ...
                window.ethereum.request({
                    ...
                    .then((txHash) =>
                        axios.post(`/backend/api/users/buy_request`, {
    ...
    async submitFormSellOrder(e) {
        ...
        setTimeout(() => {
            ...
            if (this.state.token_value > this.balanceToken) {
                toast.error(`Insufficient Balance`, {
                    position: "top-right"
                });
                return;
                ...
                web3.eth.sendTransaction(rawTransaction, (error, receipt) => {
                    ...
                    if (receipt) {
                        axios.post(`/backend/api/users/sell_request`, {
                            "address": this.state.accounts,
                            "eth_value": this.state.eth_value,
                            "token_value": this.state.token_value,
                            "price": price,
    ...
    getExchangeDetail() {
        ...
        if (this.state.accounts[0] !== undefined) {
            axios.post(`/backend/api/users/exchange_detail`, {
                "address": this.state.accounts
            }).then((res) => {
                this.codeData = res.data.code
                if (this.codeData === true) {
                    this.setState({
                        ...
                        clerk_usd_price: res.data.clerk_usd_price,
                        rcr_usd_price: res.data.rcr_usd_price
                    });
                    // console.log(this.state.exchangeList);
    ...
    getopenordersDetail() {
        if (this.state.accounts !== undefined) {
            axios.post(`/backend/api/users/my_open_order`, {
                "address": this.state.accounts
            }).then((res) => {
    ...
    getCloseOrderDetail() {
        if (this.state.accounts !== undefined) {
            axios.post(`/backend/api/users/my_close_order`, {
                "address": this.state.accounts
            }).then((res) => {
    ...
    buyOrderList() {
        axios.get(`/backend/api/users/buy_order`).then((res) => {
    ...
    sellOrderList() {
        axios.get(`/backend/api/users/sell_order`).then((res) => {
    ...
    sell_direct_order(item) {
        ...
        this.setState({
            ...
            clerk_usd_price: item.price,
            rcr_usd_price: item.price
        })
    ...
    removeOrder(item) {
        axios.post(`/backend/api/users/cancel_order`, { 'order_id': item.id }).then((res) => {
                        ...
                        <div className="bp3-card bp3-elevation-0 ExchangeBlock_exchangeBlock__2YV-i Exchange_priceChart__1phnh">
                            <div className="ExchangeBlocks_priceChartTopBar__1YSwc">
                                <span className="bp3-popover-wrapper ComboBox_comboBox__D2Ynv">
                                    <span className="bp3-popover-target bp3-popover-open">
                                        <div className="bp3-transition-container bp3-popover-enter-done" style={{ display: (this.state.open_close_box == 1) ? '' : 'none', position: 'absolute', willChange: 'transform', top: '0px', left: '0px', transform: 'translate3d(296px, 245px, 0px)' }}>
                                ...
                                <div className="ExchangeBlocks_priceChartInfo__2WGt1">
                                    <div><small>ETH Price: <span className="ExchangeBlocks_blue__3K5_O">$509.32 USD <span className="bp3-popover-wrapper"><span className="bp3-popover-target"><span className="ExchangeBlocks_green__20D86" tabindex="0">(0.3%)</span></span></span></span></small><small>GAS Price: <span className="ExchangeBlocks_blue__3K5_O">19.1 GWEI</span></small></div>
                            ...
                            {(this.state.clerk_Rcr == 'CLERK') ? <div className="ExchangeBlocks_priceChartAdditionalInfo__2bgff">
                                <span className="ExchangeBlocks_additionalInfoBox__n1oor">
                                    ...
                                    <span className="ExchangeBlocks_blue__3K5_O">0.00749249 ETH</span>
                                </span><span className="ExchangeBlocks_additionalInfoBox__n1oor">
                                    <span>CLERK Buy Wall:</span><span className="ExchangeBlocks_blue__3K5_O">At lvl. 67 ~2.97 USD</span></span><span className="ExchangeBlocks_additionalInfoBox__n1oor"><span>CLERK Supply Left:</span><span className="ExchangeBlocks_blue__3K5_O">To reach lvl. 68 5454.59 CLERK</span></span>
                                    ...
                                    <span className="ExchangeBlocks_additionalInfoBox__n1oor">
                                        ...
                                        <span>RCR Supply Left:</span><span className="ExchangeBlocks_blue__3K5_O">To reach lvl. 68 5454.59 RCR</span></span>
                                        ...
                                        <div className="bp3-form-group bp3-inline TextField_textField__30ceS">
                                            ...
                                            <div className="bp3-form-content">
                                                <div className="bp3-input-group bp3-large">
                                                    <input type="text" className="bp3-input" value={this.state.token_value}
                                                        onChange={this.onChange} name="token_value" /></div>
                                            </div>
                                        </div>
                                        <div className="bp3-form-group bp3-inline TextField_textField__30ceS">
                                            <label className="bp3-label">Price for 1 {this.state.clerk_Rcr} <span className="bp3-text-muted"></span></label>
                                            <div className="bp3-form-content">
                                                <div className="bp3-input-group bp3-large">
                                                    {this.state.clerk_Rcr === 'CLERK' ? <input type="text" className="bp3-input" value={this.state.clerk_usd_price} name="clerk_usd_price" onChange={this.onChange} /> : <input type="text" className="bp3-input" value={this.state.rcr_usd_price} name="rcr_usd_price" onChange={this.onChange} />}
                                                </div>
                                            </div>
                                        </div>
                                        <div className="bp3-form-group bp3-inline TextField_textField__30ceS">
                                            ...
                                            <div className="bp3-form-content">
                                                <div className="bp3-input-group bp3-large">
                                                    ...
                                                    {this.state.clerk_Rcr === 'CLERK' ?
                                                        <input type="text" className="bp3-input" onChange={this.onChange} name="eth_value" value={this.state.eth_value} />
                                                        :
                                                        <input type="text" className="bp3-input" onChange={this.onChange} name="eth_value" value={parseFloat(this.state.token_value) * parseFloat(this.state.exchangeList?.rcr_eth_value)} />}
                                            ...
                                            <div className="bp3-form-content">
                                                <div className="bp3-input-group bp3-large">
                                                    {this.state.clerk_Rcr === 'CLERK' ?
                                                        <input type="text" className="bp3-input" onChange={this.onChange} name="eth_value" value={this.state.eth_value} />
                                                        :
                                                        <input type="text" className="bp3-input" onChange={this.onChange} name="eth_value" value={parseFloat(this.state.token_value) * parseFloat(this.state.exchangeList?.rcr_eth_value)} />} </div>
                                            </div>
                                        </div>
